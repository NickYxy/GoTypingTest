...
//
// The options field stores various configurations options for the WAL.
//
// The mu sync.RWMutes is used for concurrent access to the WAL data structure,
...
func Open(options Options) (*WAL, error) {
	...
	if len(segmentIDs) == 0 {
		segment, err := openSegmentFile(options.DirPath, options.SegmentFileExt, initialSegmentFileID, wal.blockCache)
		if err != nil {
			return nil, err
		}
		...
		for i, segId := range segmentIDs {
			segment, err := openSegmentFile(options.DirPath, options.SegmentFileExt, uint32(segId), wal.blockCache)
			if err != nil {
				return nil, err
			}
...
// Only when there is only one active segment file, and it is empty,
// the WAL is empty.
