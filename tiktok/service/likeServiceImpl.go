...
func (like *LikeServiceImpl) IsFavourite(videoId int64, userId int64) (bool, error) {
	...
	if n, err := redis.RdbLikeUserId.Exists(strUserId).Result(); n > 0 {
		//如果有问题，说明查询redis失败,返回默认false,返回错误信息
		if err != nil {
			log.Printf("方法:IsFavourite RedisLikeUserId query key失败：%v", err)
			return false, err
		}
		exist, err1 := redis.RdbLikeUserId.SIsMember(strUserId, videoId).Result()
		//如果有问题，说明查询redis失败,返回默认false,返回错误信息
		if err1 != nil {
		...
		if n, err := redis.RdbLikeVideoId.Exists(strVideoId).Result(); n > 0 {
			//如果有问题，说明查询redis失败,返回默认false,返回错误信息
			if err != nil {
				log.Printf("方法:IsFavourite RedisLikeVideoId query key失败：%v", err)
				return false, err
			}
			exist, err1 := redis.RdbLikeVideoId.SIsMember(strVideoId, userId).Result()
			//如果有问题，说明查询redis失败,返回默认false,返回错误信息
			if err1 != nil {
			...
			if _, err := redis.RdbLikeUserId.SAdd(strUserId, config.DefaultRedisValue).Result(); err != nil {
				log.Printf("方法:IsFavourite RedisLikeUserId add value失败")
				redis.RdbLikeUserId.Del(strUserId)
				return false, err
			}
			//给键值设置有效期，类似于gc机制
			_, err := redis.RdbLikeUserId.Expire(strUserId,
			...
			for _, likeVideoId := range videoIdList {
				redis.RdbLikeUserId.SAdd(strUserId, likeVideoId)
			}
			//查询Redis LikeUserId,key：strUserId中是否存在value:videoId,存在返回true,不存在返回false
			exist, err2 := redis.RdbLikeUserId.SIsMember(strUserId, videoId).Result()
...
func (like *LikeServiceImpl) FavouriteCount(videoId int64) (int64, error) {
	...
	if n, err := redis.RdbLikeVideoId.Exists(strVideoId).Result(); n > 0 {
		...
		//获取集合中userId个数
		count, err1 := redis.RdbLikeVideoId.SCard(strVideoId).Result()
		...
		if _, err := redis.RdbLikeVideoId.SAdd(strVideoId, config.DefaultRedisValue).Result(); err != nil {
			log.Printf("方法:FavouriteCount RedisLikeVideoId add value失败")
			redis.RdbLikeVideoId.Del(strVideoId)
			return 0, err
		}
		//给键值设置有效期，类似于gc机制
		_, err := redis.RdbLikeVideoId.Expire(strVideoId,
			time.Duration(config.OneMonth)*time.Second).Result()
		if err != nil {
		...
		for _, likeUserId := range userIdList {
			redis.RdbLikeVideoId.SAdd(strVideoId, likeUserId)
		}
		//再通过set集合中userId个数,获取点赞数量
		count, err2 := redis.RdbLikeVideoId.SCard(strVideoId).Result()
		//如果有问题，说明操作redis失败,返回默认0,返回错误信息
		if err2 != nil {
...
func (like *LikeServiceImpl) FavouriteAction(userId int64, videoId int64, actionType int32) error {
	...
	if actionType == config.LikeAction {
		...
		if n, err := redis.RdbLikeUserId.Exists(strUserId).Result(); n > 0 {
			...
			if _, err1 := redis.RdbLikeUserId.SAdd(strUserId, videoId).Result(); err1 != nil {
			...
			//保证redis与mysql数据一致性
			for _, likeVideoId := range videoIdList {
				if _, err1 := redis.RdbLikeUserId.SAdd(strUserId, likeVideoId).Result(); err1 != nil {
					log.Printf("方法:FavouriteAction RedisLikeUserId add value失败")
					redis.RdbLikeUserId.Del(strUserId)
					return err1
				}
			...
			//这样操作理由同上
			if _, err2 := redis.RdbLikeUserId.SAdd(strUserId, videoId).Result(); err2 != nil {
		...
		if n, err := redis.RdbLikeVideoId.Exists(strVideoId).Result(); n > 0 {
			...
			if _, err1 := redis.RdbLikeVideoId.SAdd(strVideoId, userId).Result(); err1 != nil {
			...
			//保证redis与mysql数据一致性
			for _, likeUserId := range userIdList {
				if _, err1 := redis.RdbLikeVideoId.SAdd(strVideoId, likeUserId).Result(); err1 != nil {
					log.Printf("方法:FavouriteAction RedisLikeVideoId add value失败")
					redis.RdbLikeVideoId.Del(strVideoId)
					return err1
				}
			...
			//这样操作理由同上
			if _, err2 := redis.RdbLikeVideoId.SAdd(strVideoId, userId).Result(); err2 != nil {
		...
		if n, err := redis.RdbLikeUserId.Exists(strUserId).Result(); n > 0 {
			...
			if _, err1 := redis.RdbLikeUserId.SRem(strUserId, videoId).Result(); err1 != nil {
			...
			//这样操作理由同上
			if _, err2 := redis.RdbLikeUserId.SRem(strUserId, videoId).Result(); err2 != nil {
		...
		if n, err := redis.RdbLikeVideoId.Exists(strVideoId).Result(); n > 0 {
			...
			if _, err1 := redis.RdbLikeVideoId.SRem(strVideoId, userId).Result(); err1 != nil {
			...
			//这样操作理由同上
			if _, err2 := redis.RdbLikeVideoId.SRem(strVideoId, userId).Result(); err2 != nil {
...
func (like *LikeServiceImpl) GetFavouriteList(userId int64, curId int64) ([]Video, error) {
	...
	if n, err := redis.RdbLikeUserId.Exists(strUserId).Result(); n > 0 {
		...
		//获取集合中全部videoId
		videoIdList, err1 := redis.RdbLikeUserId.SMembers(strUserId).Result()
		//如果有问题，说明查询redis失败,返回默认nil,返回错误信息
		if err1 != nil {
		...
		//保证redis与mysql数据一致性
		for _, likeVideoId := range videoIdList {
			if _, err2 := redis.RdbLikeUserId.SAdd(strUserId, likeVideoId).Result(); err2 != nil {
				log.Printf("方法:GetFavouriteList RedisLikeUserId add value失败")
				redis.RdbLikeUserId.Del(strUserId)
				return nil, err2
			}
...
func (like *LikeServiceImpl) FavouriteVideoCount(userId int64) (int64, error) {
	...
	if n, err := redis.RdbLikeUserId.Exists(strUserId).Result(); n > 0 {
		...
		if err != nil {
			...
			count, err1 := redis.RdbLikeUserId.SCard(strUserId).Result()
			//如果有问题，说明操作redis失败,返回默认0,返回错误信息
			if err1 != nil {
		...
		count, err2 := redis.RdbLikeUserId.SCard(strUserId).Result()
		//如果有问题，说明操作redis失败,返回默认0,返回错误信息
		if err2 != nil {
