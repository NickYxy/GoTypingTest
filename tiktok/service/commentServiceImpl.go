...
func (c CommentServiceImpl) CountFromVideoId(videoId int64) (int64, error) {
	//先在缓存中查
	cnt, err := redis.RdbVCid.SCard(strconv.FormatInt(videoId, 10)).Result()
	...
	go func() {
		...
		_, _err := redis.RdbVCid.SAdd(strconv.Itoa(int(videoId)), config.DefaultRedisValue).Result()
		if _err != nil { //若存储redis失败，则直接返回
			log.Println("redis save one vId - cId 0 failed")
			return
		}
		//设置key值过期时间
		_, err := redis.RdbVCid.Expire(strconv.Itoa(int(videoId)),
...
func (c CommentServiceImpl) DelComment(commentId int64) error {
	...
	n, err := redis.RdbCVid.Exists(strconv.FormatInt(commentId, 10)).Result()
	if err != nil {
		log.Println(err)
	}
	if n > 0 { //在缓存中有此值，则找出来删除，然后返回
		vid, err1 := redis.RdbCVid.Get(strconv.FormatInt(commentId, 10)).Result()
		if err1 != nil { //没找到，返回err
			log.Println("redis find CV err:", err1)
		}
		//删除，两个redis都要删除
		del1, err2 := redis.RdbCVid.Del(strconv.FormatInt(commentId, 10)).Result()
		if err2 != nil {
			log.Println(err2)
		}
		del2, err3 := redis.RdbVCid.SRem(vid, strconv.FormatInt(commentId, 10)).Result()
		if err3 != nil {
			log.Println(err3)
		}
...
func (c CommentServiceImpl) GetList(videoId int64, userId int64) ([]CommentInfo, error) {
	...
	*/
	...
	go func() {
		//1.先在缓存中查此视频是否已有评论列表
		cnt, err1 := redis.RdbVCid.SCard(strconv.FormatInt(videoId, 10)).Result()
		...
		//设置key值过期时间
		_, err2 := redis.RdbVCid.Expire(strconv.Itoa(int(videoId)),
			time.Duration(config.OneMonth)*time.Second).Result()
		if err2 != nil {
			log.Println("redis save one vId - cId expire failed")
		}
