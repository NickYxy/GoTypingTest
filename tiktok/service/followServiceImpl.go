...
func (*FollowServiceImp) IsFollowing(userId int64, targetId int64) (bool, error) {
	// 先查Redis里面是否有此关系。
	if flag, err := redis.RdbFollowingPart.SIsMember(strconv.Itoa(int(userId)), targetId).Result(); flag {
		// 重现设置过期时间。
		redis.RdbFollowingPart.Expire(strconv.Itoa(int(userId)), config.ExpireTime)
		return true, err
	}
	// SQL 查询。
...
func addRelationToRedis(userId int, targetId int) {
	...
	redis.RdbFollowingPart.SAdd(strconv.Itoa(int(userId)), -1)
	// 将查询到的关注关系注入Redis.
	redis.RdbFollowingPart.SAdd(strconv.Itoa(int(userId)), targetId)
	// 更新过期时间。
	redis.RdbFollowingPart.Expire(strconv.Itoa(int(userId)), config.ExpireTime)
}
...
func (*FollowServiceImp) GetFollowerCnt(userId int64) (int64, error) {
	// 查Redis中是否已经存在。
	if cnt, err := redis.RdbFollowers.SCard(strconv.Itoa(int(userId))).Result(); cnt > 0 {
		// 更新过期时间。
		redis.RdbFollowers.Expire(strconv.Itoa(int(userId)), config.ExpireTime)
		return cnt - 1, err
	}
	// SQL中查询。
...
func addFollowersToRedis(userId int, ids []int64) {
	redis.RdbFollowers.SAdd(strconv.Itoa(userId), -1)
	for i, id := range ids {
		redis.RdbFollowers.SAdd(strconv.Itoa(userId), id)
		redis.RdbFollowingPart.SAdd(strconv.Itoa(int(id)), userId)
		redis.RdbFollowingPart.SAdd(strconv.Itoa(int(id)), -1)
		// 更新部分关注者的时间
		redis.RdbFollowingPart.Expire(strconv.Itoa(int(id)),
			config.ExpireTime+time.Duration((i%10)<<8))
	}
	// 更新followers的过期时间。
	redis.RdbFollowers.Expire(strconv.Itoa(userId), config.ExpireTime)
...
func (*FollowServiceImp) GetFollowingCnt(userId int64) (int64, error) {
	// 查看Redis中是否有关注数。
	if cnt, err := redis.RdbFollowing.SCard(strconv.Itoa(int(userId))).Result(); cnt > 0 {
		// 更新过期时间。
		redis.RdbFollowing.Expire(strconv.Itoa(int(userId)), config.ExpireTime)
		return cnt - 1, err
	}
	// 用SQL查询。
...
func addFollowingToRedis(userId int, ids []int64) {
	redis.RdbFollowing.SAdd(strconv.Itoa(userId), -1)
	for i, id := range ids {
		redis.RdbFollowing.SAdd(strconv.Itoa(userId), id)
		redis.RdbFollowingPart.SAdd(strconv.Itoa(userId), id)
		redis.RdbFollowingPart.SAdd(strconv.Itoa(userId), -1)
		// 更新过期时间
		redis.RdbFollowingPart.Expire(strconv.Itoa(userId),
			config.ExpireTime+time.Duration((i%10)<<8))
	}
	// 更新following的过期时间
	redis.RdbFollowing.Expire(strconv.Itoa(userId), config.ExpireTime)
}
...
func updateRedisWithAdd(userId int64, targetId int64) (bool, error) {
	...
	*/
	...
	if cnt, _ := redis.RdbFollowers.SCard(targetIdStr).Result(); 0 != cnt {
		redis.RdbFollowers.SAdd(targetIdStr, userId)
		redis.RdbFollowers.Expire(targetIdStr, config.ExpireTime)
	}
	// step2
	followingUserIdStr := strconv.Itoa(int(userId))
	if cnt, _ := redis.RdbFollowing.SCard(followingUserIdStr).Result(); 0 != cnt {
		redis.RdbFollowing.SAdd(followingUserIdStr, targetId)
		redis.RdbFollowing.Expire(followingUserIdStr, config.ExpireTime)
	}
	// step3
	followingPartUserIdStr := followingUserIdStr
	redis.RdbFollowingPart.SAdd(followingPartUserIdStr, targetId)
	// 可能是第一次给改用户加followingPart的关注者，需要加上-1防止脏读。
	redis.RdbFollowingPart.SAdd(followingPartUserIdStr, -1)
	redis.RdbFollowingPart.Expire(followingPartUserIdStr, config.ExpireTime)
	return true, nil
}
...
func updateRedisWithDel(userId int64, targetId int64) (bool, error) {
	...
	*/
	...
	targetIdStr := strconv.Itoa(int(targetId))
	if cnt, _ := redis.RdbFollowers.SCard(targetIdStr).Result(); 0 != cnt {
		redis.RdbFollowers.SRem(targetIdStr, userId)
		redis.RdbFollowers.Expire(targetIdStr, config.ExpireTime)
	}
	// step2
	followingIdStr := strconv.Itoa(int(userId))
	if cnt, _ := redis.RdbFollowing.SCard(followingIdStr).Result(); 0 != cnt {
		redis.RdbFollowing.SRem(followingIdStr, targetId)
		redis.RdbFollowing.Expire(followingIdStr, config.ExpireTime)
	}
	// step3
	followingPartUserIdStr := followingIdStr
	if cnt, _ := redis.RdbFollowingPart.Exists(followingPartUserIdStr).Result(); 0 != cnt {
		redis.RdbFollowingPart.SRem(followingPartUserIdStr, targetId)
		redis.RdbFollowingPart.Expire(followingPartUserIdStr, config.ExpireTime)
	}
	return true, nil
}
...
func setRedisFollowing(userId int64, users []User) {
	...
	*/
	// 加上-1防止脏读
	followingIdStr := strconv.Itoa(int(userId))
	redis.RdbFollowing.SAdd(followingIdStr, -1)
	// 设置过期时间
	redis.RdbFollowing.Expire(followingIdStr, config.ExpireTime)
	for i, user := range users {
		redis.RdbFollowing.SAdd(followingIdStr, user.Id)

		redis.RdbFollowingPart.SAdd(followingIdStr, user.Id)
		redis.RdbFollowingPart.SAdd(followingIdStr, -1)
		// 随机设置过期时间
		redis.RdbFollowingPart.Expire(followingIdStr, config.ExpireTime+
...
func setRedisFollowers(userId int64, users []User) {
	...
	*/
	// 加上-1防止脏读。
	followersIdStr := strconv.Itoa(int(userId))
	redis.RdbFollowers.SAdd(followersIdStr, -1)
	// 设置过期时间
	redis.RdbFollowers.Expire(followersIdStr, config.ExpireTime)
	for i, user := range users {
		redis.RdbFollowers.SAdd(followersIdStr, user.Id)

		userUserIdStr := strconv.Itoa(int(user.Id))
		redis.RdbFollowingPart.SAdd(userUserIdStr, userId)
		redis.RdbFollowingPart.SAdd(userUserIdStr, -1)
		// 随机更新过期时间
		redis.RdbFollowingPart.Expire(userUserIdStr, config.ExpireTime+
			time.Duration((i%10)<<8))

		if user.IsFollow {
			redis.RdbFollowingPart.SAdd(followersIdStr, user.Id)
			redis.RdbFollowingPart.SAdd(followersIdStr, -1)
			redis.RdbFollowingPart.Expire(followersIdStr, config.ExpireTime+
				time.Duration((i%10)<<8))
		}
	}
}
